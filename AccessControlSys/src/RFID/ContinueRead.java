        ...
        case SerialPortEvent.DATA_AVAILABLE:// 当有可用数据时读取数据
            ...
            try {
                ...
                while (inputStream!=null&&inputStream.available() > 0) {
                    ...
                    if (numBytes > 0) {
                    	msgQueue.add(DatatypeConverter.printHexBinary(readBuffer));
                        readBuffer = new byte[5];// 重新构造缓冲对象，否则有可能会影响接下来接收的数据
                    } else {
        ...
        try {
            ...
            while (true) {
                ...
                if (msgQueue!=null&&msgQueue.size() > 0) {
                	...
                	String uid=msgQueue.take();
                    System.out.println("读入："+uid);
                    try {
                    	test.uid=uid;
					    new test().start();
					} catch (Exception e) {
